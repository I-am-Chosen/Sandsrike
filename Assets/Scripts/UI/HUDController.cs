...
namespace HEAVYART.TopDownShooter.Netcode
{
    public class HUDController : MonoBehaviour
    {
        ...
        public void UpdatePowerUpIndicators(CommandsControlSystem commandsControlSystem, bool updateProgress)
        {
            //Accuracy powerup indicator
            ModifierScriptableObject accuracyModifier = commandsControlSystem.GetModifier<AccuracyModifier>();
            UpdateIndicator(accuracyPowerUpIndicator, accuracyModifier, updateProgress);

            //FireRate powerup indicator
            ModifierScriptableObject fireRateModifier = commandsControlSystem.GetModifier<FireRateModifier>();
            UpdateIndicator(fireRatePowerUpIndicator, fireRateModifier, updateProgress);

            //Speed powerup indicator
            ModifierScriptableObject speedModifier = commandsControlSystem.GetModifier<SpeedModifier>();
            UpdateIndicator(speedPowerUpIndicator, speedModifier, updateProgress);
        }

        private void UpdateIndicator(Image indicator, ModifierScriptableObject activeModifierData, bool updateProgress)
        {
